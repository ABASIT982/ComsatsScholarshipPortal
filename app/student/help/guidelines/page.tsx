...
export default function StudentGuidelinesPage() {
  ...
  const fetchGuidelines = async () => {
    try {
      const response = await fetch('/api/help/guidelines')
  ...
  const categories = ['all', ...new Set(guidelines.map(g => g.category))]

  const filteredItems = guidelines.filter(item => {
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Scholarship Guidelines</h1>
            <p className="text-gray-500 text-sm mt-1">Important guidelines for scholarship applications</p>
          </div>
        </div>
      </div>
